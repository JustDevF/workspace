...
public class PMenagerie extends JPanel {
	...
	private JButton btnAfficherAnimal;
	private final Action actionAfficherAnimaux = new ACtionAfficherAnimaux();
	...
	public PMenagerie() {
		...
		btnAfficherAnimal = new JButton("Afficher Animal");
		btnAfficherAnimal.setAction(actionAfficherAnimaux);
		btnAfficherAnimal.setHorizontalAlignment(SwingConstants.LEFT);
		btnAfficherAnimal.setIcon(new ImageIcon(PMenagerie.class.getResource("/images/gestion/Receipt-48.png")));
		btnAfficherAnimal.setForeground(Color.BLACK);
		btnAfficherAnimal.setFont(new Font("Roboto Lt", Font.PLAIN, 14));
		GridBagConstraints gbc_btnAfficherAnimal = new GridBagConstraints();
		gbc_btnAfficherAnimal.fill = GridBagConstraints.HORIZONTAL;
		gbc_btnAfficherAnimal.insets = new Insets(0, 0, 5, 0);
		gbc_btnAfficherAnimal.gridx = 0;
		gbc_btnAfficherAnimal.gridy = 4;
		btnAfficherAnimal.setCursor(new java.awt.Cursor(java.awt.Cursor.HAND_CURSOR));
		panel_menu.add(btnAfficherAnimal, gbc_btnAfficherAnimal);
		UI.deshabillerBouton(btnAfficherAnimal, "Receipt");
		
		//Action supprimer
		btnSupprimer = new JButton("Supprimer un animal");
		btnSupprimer.setAction(actionSupprimerAnimal);
  ...
  //Action afficher les annimaux présents dans la liste de collection  
  	private class ACtionAfficherAnimaux extends AbstractAction {
  		private static final long serialVersionUID = 1L;
  		public ACtionAfficherAnimaux() {
  			putValue(NAME, "Afficher Animaux");
  			putValue(SHORT_DESCRIPTION, "afficher les annimaux présents dans la liste de collection  ");
  		}
  		public void actionPerformed(ActionEvent e) {
  			afficherAnimaux();
  		}
  ...
  //Action Supprimer un animal
  	private class ActionSupprimerAnimal extends AbstractAction {
  		...
  		public ActionSupprimerAnimal() {
  			...
  			putValue(SHORT_DESCRIPTION, "Supprimer un animal");
  		}
  		public void actionPerformed(ActionEvent e) {
  			supprimerAnimal();
  		}
	...
	//méthode pour afficher les animaux   ..à travailler
	public void afficherAnimaux() {
		...
		//Déclaration et init de la collection
		...
		for(Animal unAnimal : listAnimaux) {
			//transtypage descendant pour accéder aux prorprietés de classe fille
			//Classe fille Chat
			if(listAnimaux.get(i) instanceof Chat) {
				Chat unChat = (Chat) listAnimaux.get(i);
				data[i][0] = unAnimal.getEspece();
				data[i][1] = unChat.getNom();
				data[i][2] = unChat.getRace();
				data[i][3] = unChat.getCouleur();
				data[i][4] = unChat.getOrigine();	
				data[i][5] = "Chat";
				
			//Classe fille Chien
			} else if (listAnimaux.get(i) instanceof Chien) {
				Chien unChien = (Chien) listAnimaux.get(i);
				data[i][0] = unChien.getEspece();
				data[i][1] = unChien.getNom();
				data[i][2] = unChien.getRace();
				data[i][3] = unChien.getTypeEducation();
				data[i][4] = unChien.getEnvironnement();
				data[i][5] = "Chien";
			} 
        ...
        panel_principal.add(scrollPane, "cell 0 1 4 1,grow");
 		...
 		btnAfficherAnimal.setText("Affichage des animaux");
		
 		
	}
	

	//Méthode supprimerAnimal
	public void supprimerAnimal() {
		// instancier un nouveau recherche
		PMenagerieSupprimerAnimal supprimer = new PMenagerieSupprimerAnimal();
				
		supprimer.setActionAnnuler(actionAnnuler); 
				 
		btnRechercher.setIcon(new ImageIcon( PMenagerie.class.getResource( 
			                    "/images/gestion/Search-48.png"))); 
		//rédiriger vers le pennau de recherche
		changerPanneau(supprimer, "Recherche de client(s)"); 
	}
}
